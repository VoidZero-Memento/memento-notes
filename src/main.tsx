...
import { dismissSplash, waitAppBgPainted, waitEntryPainted } from "@/lib/splash/splash-gate";
...
if (readStoredSidebarBg()) {
  void Promise.all([waitForSplash(), waitAppBgPainted(), waitEntryPainted()]).then(dismissSplash);
} else {
  dismissSplash();
}
