...
export const useImageZoom = ({ enabled, overlayRef, imageRef }: UseImageZoomArgs) => {
  ...
  useEffect(() => {
    ...
    let transform: ImageZoomTransform = { ...IMAGE_ZOOM_IDENTITY };
    ...
    const imageSize = () => ({ width: image.offsetWidth, height: image.offsetHeight });
    ...
    const zoomAt = (nextScale: number, focal: ImageZoomPoint, smooth = false) => {
      commit(zoomAround(transform, nextScale, focal, visualCenterOf(image.getBoundingClientRect())), smooth);
    };

    const resetZoom = (smooth: boolean) => {
      transform = { ...IMAGE_ZOOM_IDENTITY };
      apply(smooth);
    };
    ...
    const toggleDoubleTap = (focal: ImageZoomPoint) => {
      if (transform.scale > ZOOMED_SCALE) resetZoom(true);
      else zoomAt(IMAGE_ZOOM_DOUBLE, focal, true);
      lastTapAt = 0;
      lastTap = null;
      markGesture();
    };
    ...
    const handleWheel = (event: WheelEvent) => {
      ...
      zoomAt(clampScale(transform.scale * Math.exp(-dy * WHEEL_ZOOM_INTENSITY)), { x: event.clientX, y: event.clientY });
    };
